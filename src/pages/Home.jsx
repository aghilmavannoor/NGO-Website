...
const Home = () => {
  ...
  return (
    <div className="overflow-hidden font-sans">
      ...
      <section className="relative h-screen flex items-center bg-[#0B2E1E] text-white overflow-hidden px-6 sm:px-12 lg:px-16">
        
        {/* Cinematic Background Image & Dual Gradient Overlays */}
        <div className="absolute inset-0 z-0">
          <img 
            src={homeHeroBg} 
            alt="School children and officials planting saplings" 
            className="w-full h-full object-cover object-center lg:object-[35%_center] select-none pointer-events-none"
          />
          {/* Left-to-right cinematic gradient to protect text readability on the left */}
          <div className="absolute inset-0 bg-gradient-to-r from-[#0B2E1E] via-[#0B2E1E]/70 to-[#0B2E1E]/10 lg:via-[#0B2E1E]/55 lg:to-transparent z-10"></div>
          
          {/* Bottom vignette to blend into the next section */}
          <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-[#0B2E1E] to-transparent z-15"></div>
        </div>

        {/* Hero Content Container */}
        <div className="max-w-7xl mx-auto relative z-20 space-y-6 sm:space-y-8 w-full text-left flex flex-col items-start px-0">
          {/* Leaf Badge */}
          <motion.div
          ...
          <motion.p
            ...
            className="text-slate-200 text-sm sm:text-lg max-w-xl font-medium leading-relaxed drop-shadow-sm"
          >
          ...
          >
            <Link to="/projects">
              <Button variant="primary" size="lg" className="bg-[#5BBE3A] hover:bg-[#4ca82f] border-transparent text-white rounded-full font-bold px-8 py-3.5 flex items-center gap-2 transition-transform hover:scale-105 duration-300 shadow-lg shadow-emerald-950/20 text-sm sm:text-base">
                <Leaf size={18} /> Explore Our Projects
              </Button>
            </Link>
            <Link to="/volunteer">
              <Button variant="outline" size="lg" className="border-white/20 bg-white/5 hover:bg-white/10 text-white rounded-full font-bold px-8 py-3.5 flex items-center gap-2 transition-transform hover:scale-105 duration-300 text-sm sm:text-base">
                <Users size={18} /> Join as Volunteer
              </Button>
            </Link>
          </motion.div>

          {/* Features list row - Horizontal 3-column grid */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.6 }}
            className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 border-t border-white/15 w-full max-w-4xl"
          >
            <div className="flex items-center gap-4">
              <div className="w-11 h-11 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0">
                <Leaf size={20} className="fill-[#5BBE3A]/10 text-[#5BBE3A]" />
              </div>
              <div>
                <h4 className="font-extrabold text-white text-sm font-display tracking-wide">Plant Trees</h4>
                <p className="text-slate-300 text-xs mt-0.5 leading-relaxed">Create a greener planet</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-11 h-11 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0">
                <Droplet size={20} className="fill-[#5BBE3A]/10 text-[#5BBE3A]" />
              </div>
              <div>
                <h4 className="font-extrabold text-white text-sm font-display tracking-wide">Save Water</h4>
                <p className="text-slate-300 text-xs mt-0.5 leading-relaxed">Protect every drop</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-11 h-11 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0">
                <Users size={20} className="fill-[#5BBE3A]/10 text-[#5BBE3A]" />
              </div>
              <div>
                <h4 className="font-extrabold text-white text-sm font-display tracking-wide">Empower Communities</h4>
                <p className="text-slate-300 text-xs mt-0.5 leading-relaxed">Together for a better tomorrow</p>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 w-80 h-80 pointer-events-none opacity-20 z-10 select-none">
